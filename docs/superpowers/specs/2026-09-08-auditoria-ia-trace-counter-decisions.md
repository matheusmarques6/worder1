...
| W4-TC-02 | aprovado; alteração de produto pendente | Contadores A: retirar exposição/labels que apresentam totais congelados como vivos; preservar colunas e dados de `ai_agents`. Rejeitada atualização materializada B e reativação de RPC arquivada. | Métrica futura deve declarar evento real e janela, conforme denominadores abaixo. | Nenhuma superfície ativa dos totais legados confirmada; nenhum agregado novo por agente autorizado. | Produto/controlador; Sol nos leitores confirmados; Astra revisa. | Aprovou condicionalmente A e a preservação do histórico. | 2026-09-15 |
