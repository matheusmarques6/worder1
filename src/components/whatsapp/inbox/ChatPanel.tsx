...
function MessageStatus({ status }: { status: InboxMessage['status'] }) {
  switch (status) {
    case 'pending': return <Clock className="w-3.5 h-3.5 text-white/40" />
    case 'sent': return <Check className="w-3.5 h-3.5 text-white/50" />
    case 'delivered': return <CheckCheck className="w-3.5 h-3.5 text-white/50" />
    case 'read': return <CheckCheck className="w-3.5 h-3.5 text-[#53bdeb]" />
    case 'played': return <CheckCheck className="w-3.5 h-3.5 text-[#53bdeb]" />
    case 'failed': return <AlertCircle className="w-3.5 h-3.5 text-red-400" />
    default: return <Check className="w-3.5 h-3.5 text-white/50" />
  }
}
      ...
      <div className={`max-w-[85%] sm:max-w-[75%] lg:max-w-[65%] ${isOutbound ? 'items-end' : 'items-start'}`}>
        <div className={`relative rounded-lg px-3 py-2 shadow-sm ${
          isFailed
            ...
            : isPending
              ? 'bg-[#005c4b] opacity-70'
              : isOutbound
                ? isBot ? 'bg-gradient-to-r from-[#005c4b] to-[#025144]' : 'bg-[#005c4b]'
                : 'bg-[#202c33]'
        } ${isOutbound ? 'rounded-tr-sm' : 'rounded-tl-sm'}`}>
          {isBot && <div className="absolute top-1.5 right-1.5"><Bot className="w-3 h-3 text-white/40" /></div>}
          ...
          {message.content && message.message_type !== 'audio' && message.message_type !== 'ptt' && (
            <p className={`text-[13px] sm:text-sm whitespace-pre-wrap break-words leading-relaxed ${isOutbound ? 'text-[#e9edef]' : 'text-[#e9edef]'}`}>
              {message.content}
            </p>
          )}
...
function DateSeparator({ date }: { date: string }) {
  ...
  return (
    <div className="flex justify-center my-3 sm:my-4">
      <span className="text-[10px] sm:text-[11px] text-[#8696a0] bg-[#182229] px-3 py-1 rounded-md shadow-sm uppercase tracking-wide">
        {formatDate(date)}
      </span>
    </div>
  )
...
export function ChatPanel({
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0">
      ...
      {/* Header - Responsive */}
      <div className="flex items-center justify-between p-3 sm:p-4 border-b border-dark-700/50 bg-dark-800/50 backdrop-blur-sm sticky top-0 z-10">
        <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-1">
          <button onClick={onBack} className="lg:hidden p-2 -ml-1 rounded-lg hover:bg-dark-700/50 text-dark-400 flex-shrink-0">
          ...
          <div className="min-w-0 flex-1">
            <h3 className="font-semibold text-white text-sm sm:text-base truncate">{contactName}</h3>
            <p className="text-[10px] sm:text-xs text-dark-400 truncate">{formatPhone(conversation.phone_number)}</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-1 sm:gap-2 flex-shrink-0">
          ...
          >
            ...
            <span className="hidden sm:inline">{conversation.is_bot_active ? 'Bot' : 'Off'}</span>
          </button>

          <button className="hidden sm:flex p-2 rounded-lg hover:bg-dark-700/50 text-dark-400 hover:text-white">
          ...
          <button
            ...
            className="hidden xl:flex p-2 rounded-lg hover:bg-dark-700/50 text-dark-400 hover:text-white"
          >
      ...
      <div className="p-2 sm:p-3 bg-[#202c33]">
        {/* 24h Window Warning */}
        {conversation.can_send_template_only && (
          <div className="flex items-center gap-2 p-2 sm:p-3 mb-2 bg-yellow-500/10 border border-yellow-500/20 rounded-lg">
            <AlertCircle className="w-4 h-4 text-yellow-400 flex-shrink-0" />
            <span className="text-xs sm:text-sm text-yellow-400">Janela expirada. Use template.</span>
            <button className="ml-auto text-xs sm:text-sm text-green-400 font-medium">
              Template
            </button>
          </div>
        )}
          ...
          <div className="flex items-end gap-1.5 sm:gap-2">
            ...
            <div className="relative flex-shrink-0">
              ...
              {showAttachMenu && (
                <>
                  ...
                  <div className="absolute bottom-full left-0 mb-2 bg-[#233138] border border-[#2a3942] rounded-xl shadow-2xl overflow-hidden z-50 min-w-[160px]">
                    <button
                      onClick={() => handleFileTypeSelect('image')}
                      className="flex items-center gap-3 w-full px-4 py-3 hover:bg-[#2a3942] text-left"
                    >
                    ...
                    >
                      ...
                      <span className="text-[#e9edef] text-sm">Vídeos</span>
                    </button>
                    <button
            ...
            <div className="flex-1">
              <textarea
                ...
                disabled={isSending}
                rows={1}
                className="w-full px-3 sm:px-4 py-2 sm:py-2.5 bg-[#2a3942] rounded-lg text-[#e9edef] text-sm placeholder-[#8696a0] focus:outline-none resize-none disabled:opacity-50"
            ...
            {input.trim() ? (
              <button
                onClick={handleSend}
                disabled={isSending}
                className="p-2 sm:p-2.5 rounded-full bg-[#00a884] text-white hover:bg-[#00997a] disabled:opacity-50 disabled:cursor-not-allowed flex-shrink-0"
              >
              ...
              <button
                ...
                className="p-2 sm:p-2.5 rounded-full bg-[#00a884] text-white hover:bg-[#00997a] flex-shrink-0"
              >
