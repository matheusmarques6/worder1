...
// Admin client para criar/deletar usuários (PRECISA service role key)
...
export async function GET(request: NextRequest) {
  ...
  try {
    // RLS filtra automaticamente - primeiro tenta nova tabela 'agents'
    let { data, error } = await supabase
      .from('agents')
      .select('*')
      .order('created_at', { ascending: false });

    // Se nova tabela não existe, usa antiga
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // CRIAR AGENTE
    const { name, email, password, role = 'agent', ai_config, permissions, force_password_change = true } = body;
    ...
    if (agentType === 'human') {
      ...
      // Verificar limite de 3 agentes humanos - RLS filtra
      const { count } = await supabase
        .from('agents')
        .select('*', { count: 'exact', head: true })
        .eq('type', 'human');
      
      let humanCount = count || 0;
      ...
      if (!finalPassword) {
        ...
        if (!passwordValidation.valid) {
          return NextResponse.json({ error: 'Senha fraca', details: passwordValidation.errors }, { status: 400 });
        }
      }

      // Criar usuário no Auth - PRECISA admin client
      ...
      if (existingAuthUser) {
        ...
        if (existingProfile && existingProfile.organization_id !== orgId) {
          return NextResponse.json({ error: 'Este email já está registrado em outra organização' }, { status: 409 });
        }
        ...
        const { data: authData, error: authError } = await adminClient.auth.admin.createUser({
          ...
          email_confirm: true,
          user_metadata: { name, role: 'agent', organization_id: orgId, is_agent: true }
        });
        ...
        let { error: profileError } = await supabase
          ...
          .upsert({
            ...
            organization_id: orgId, role: profileRole,
      ...
      // Criar agente
      const agentData = {
        organization_id: orgId, user_id: authUserId, type: 'human',
        name, email, is_active: true, status: 'offline',
        total_conversations: 0, total_messages: 0,
        created_at: new Date().toISOString(), updated_at: new Date().toISOString(),
      };

      let { data: agent, error: agentError } = await supabase.from('agents').insert(agentData).select().single();

      if (agentError && agentError.code === '42P01') {
        const result = await supabase
          .from('whatsapp_agents')
          .insert({ organization_id: orgId, user_id: authUserId, name, email, role, is_active: true, is_available: false })
          .select().single();
        agent = result.data;
      ...
      if (agentError) {
        ...
        if (!existingAuthUser) await adminClient.auth.admin.deleteUser(authUserId);
      ...
      const permissionsData = {
        ...
        can_transfer_conversations: permissions?.can_transfer_conversations ?? true,
        can_use_ai_suggestions: permissions?.can_use_ai_suggestions ?? true,
        can_send_media: permissions?.can_send_media ?? true,
        can_use_quick_replies: permissions?.can_use_quick_replies ?? true,
        can_access_crm: permissions?.can_access_crm || false,
        can_access_pipelines: permissions?.can_access_pipelines ?? false,
      ...
      await adminClient.auth.admin.updateUserById(authUserId, {
        user_metadata: { name, role: 'agent', organization_id: orgId, is_agent: true, agent_id: agent.id }
      });

      return NextResponse.json({ 
        agent, password_generated: passwordGenerated,
        temporary_password: passwordGenerated ? finalPassword : undefined,
        message: passwordGenerated ? 'Agente criado com senha temporária.' : 'Agente criado com sucesso.'
      }, { status: 201 });
    }
    ...
    if (agentType === 'ai') {
      ...
      const agentData = {
        organization_id: orgId, type: 'ai', name, email: null,
        is_active: true, status: 'online', ai_config,
        total_conversations: 0, total_messages: 0,
        created_at: new Date().toISOString(), updated_at: new Date().toISOString(),
      };
      ...
      if (agentError && agentError.code === '42P01') {
        const result = await supabase
          .from('whatsapp_agents')
          .insert({ organization_id: orgId, name, type: 'ai', is_active: true, is_available: true })
          .select().single();
        agent = result.data;
        agentError = result.error;
      }
      ...
      if (agent) {
        await supabase.from('ai_agent_configs').upsert({
          ...
          use_whatsapp: true, always_active: false, only_when_no_human: true,
          created_at: new Date().toISOString(), updated_at: new Date().toISOString(),
        }, { onConflict: 'agent_id' });
      }
...
export async function PATCH(request: NextRequest) {
  ...
  try {
    ...
    let { data, error } = await supabase.from('agents').update(filtered).eq('id', id).select().single();

    if (error && error.code === '42P01') {
      const result = await supabase.from('whatsapp_agents').update(filtered).eq('id', id).select().single();
      data = result.data;
      error = result.error;
    }
    ...
    if (updateData.permissions && data) {
      await supabase.from('agent_permissions').upsert({
        agent_id: data.id, ...updateData.permissions, updated_at: new Date().toISOString(),
      }, { onConflict: 'agent_id' });
    }
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    ...
    // Verificar chats ativos - RLS filtra
    const { count } = await supabase
      .from('chat_assignments')
      .select('*', { count: 'exact', head: true })
      .eq('agent_id', id)
    ...
    if (count === null) {
      ...
      if (oldCount && oldCount > 0) {
        return NextResponse.json({ error: 'Agente possui chats ativos.' }, { status: 400 });
      }
    } else if (count > 0) {
      return NextResponse.json({ error: 'Agente possui chats ativos.' }, { status: 400 });
    }

    // Buscar agente para pegar user_id - RLS filtra
    const { data: agent } = await supabase.from('agents').select('user_id, type').eq('id', id).single();
    ...
    // Deletar agente - RLS filtra
    let { error } = await supabase.from('agents').delete().eq('id', id);

    if (error && error.code === '42P01') {
      const result = await supabase.from('whatsapp_agents').delete().eq('id', id);
      error = result.error;
    }

    if (error) throw error;

    // Se era agente humano com user_id, deletar também do Auth
