...
export async function POST(request: NextRequest) {
  ...
  if (storeIdParam) {
    ...
    // Sem storeId só serve a ÚNICA loja ativa da organização — nunca a
    // mais nova, que sincronizava as finanças da loja errada.
    const { pickStore, pickStoreError } = await import('@/lib/stores/pick-store');
    const picked = await pickStore<any>(supabase, { orgIds: [organizationId], select: '*' });
    if (!picked.store) {
