  ...
  if (!automation) {
    return (
      <div className="flex flex-col items-center justify-center py-32 text-center">
        ...
        <h3 className="text-lg font-semibold text-white mb-1">Automação não encontrada</h3>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        >
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-white mt-1">{status.label}</p>
            </div>
        ...
        >
          <div className="flex items-start justify-between">
            <div>
              <p className="text-xs text-zinc-500 font-medium">Trigger</p>
              <p className="text-lg font-bold text-white mt-1 truncate">{triggerLabel}</p>
            </div>
        ...
        >
          <div className="flex items-start justify-between">
            <div>
              <p className="text-xs text-zinc-500 font-medium">Etapas</p>
        ...
        >
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-lg font-bold text-white mt-1">{new Date(automation.updated_at).toLocaleDateString('pt-BR')}</p>
            </div>
        ...
        <div className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-6">
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-xs text-zinc-500 mt-0.5">Nós configurados nesta automação</p>
            </div>
                ...
                <div key={node.id || i}>
                  <div className="flex items-center gap-4 p-4 bg-zinc-800/30 rounded-xl hover:bg-zinc-800/50 transition-colors">
                    <div className="w-10 h-10 rounded-xl bg-zinc-500/10 flex items-center justify-center flex-shrink-0">
                      <Lightning size={20} className="text-zinc-400" weight="fill" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <span className="text-xs text-zinc-600 font-mono">#{i + 1}</span>
                        <p className="text-sm text-white font-medium">{nodeName}</p>
                        <span className="text-[10px] px-2 py-0.5 rounded-full bg-white/[0.06] text-zinc-500 font-medium">
                          {nodeType}
                        </span>
                      </div>
                      {nodeDesc && <p className="text-xs text-zinc-500 mt-0.5">{nodeDesc}</p>}
                    </div>
                  </div>
                  {i < nodes.length - 1 && (
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <p className="text-sm text-white mt-0.5">{triggerLabel}</p>
            </div>
            <div>
              <p className="text-xs text-zinc-500">Etapas</p>
              <p className="text-sm text-white mt-0.5">{nodes.length} etapas</p>
            </div>
            <div>
              <p className="text-xs text-zinc-500">Criada em</p>
              <p className="text-sm text-white mt-0.5">{new Date(automation.created_at).toLocaleDateString('pt-BR')}</p>
            </div>
            <div>
              <p className="text-xs text-zinc-500">Última edição</p>
              <p className="text-sm text-white mt-0.5">{new Date(automation.updated_at).toLocaleDateString('pt-BR')}</p>
            </div>
          </div>
          <div className="w-full h-px bg-zinc-800" />
          <div>
            <p className="text-xs text-zinc-500">ID da Automação</p>
            <p className="text-sm text-zinc-400 font-mono mt-0.5">{params.id}</p>
          </div>
        </div>
      </div>
    </div>
