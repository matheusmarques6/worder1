...
export default function PhonebooksPage() {
  ...
  return (
    <div className="flex h-[calc(100vh-120px)] gap-6">
      <div className="w-80 flex-shrink-0 bg-white rounded-2xl border border-slate-800/50 flex flex-col">
        <div className="p-4 border-b border-slate-800/50">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-semibold text-gray-900">Listas</h2>
        ...
        <div className="flex-1 overflow-y-auto p-2">
          ...
          : phonebooks.length === 0 ? <div className="flex flex-col items-center justify-center h-32 text-gray-400"><Users className="w-10 h-10 mb-2 opacity-50" /><p className="text-sm">Nenhuma lista</p></div>
          : phonebooks.map(pb => (
            <button key={pb.id} onClick={() => setSelectedPhonebook(pb)}
              className={`w-full p-3 rounded-xl text-left transition-colors mb-2 ${selectedPhonebook?.id === pb.id ? 'bg-violet-600/20 border border-violet-500/30' : 'hover:bg-gray-50'}`}>
              <div className="flex items-center gap-3">
                <div className="p-2 rounded-lg bg-gray-50"><Users className="w-4 h-4 text-violet-400" /></div>
                <div className="flex-1 min-w-0"><p className="font-medium text-gray-900 truncate">{pb.name}</p><p className="text-xs text-gray-500">{pb.contact_count} contatos</p></div>
              </div>
            </button>
          ))}
        </div>
      ...
      <div className="flex-1 bg-white rounded-2xl border border-slate-800/50 flex flex-col">
        {selectedPhonebook ? (
          <>
            <div className="p-4 border-b border-slate-800/50">
              <div className="flex items-center justify-between mb-4">
                <div><h2 className="text-lg font-semibold text-gray-900">{selectedPhonebook.name}</h2><p className="text-sm text-gray-500">{selectedPhonebook.contact_count} contatos</p></div>
                <div className="flex gap-2">
                  <button onClick={() => setShowImportModal(true)} className="flex items-center gap-2 px-3 py-2 bg-gray-50 text-gray-600 rounded-lg hover:bg-gray-100 text-sm"><Upload className="w-4 h-4" /> Importar</button>
              ...
              <div className="relative">
                ...
                <input type="text" placeholder="Buscar contatos..." value={search} onChange={e => setSearch(e.target.value)}
                  className="w-full pl-10 pr-4 py-2.5 bg-gray-50 border border-slate-700/50 rounded-xl text-gray-900 placeholder-slate-400 focus:outline-none focus:border-violet-500/50" />
              </div>
            </div>
            <div className="flex-1 overflow-y-auto p-4">
              ...
              : filteredContacts.length === 0 ? <div className="flex flex-col items-center justify-center h-32 text-gray-400"><Users className="w-10 h-10 mb-2 opacity-50" /><p className="text-sm">Nenhum contato</p></div>
              : <div className="space-y-2">{filteredContacts.map(contact => (
                <div key={contact.id} className="p-3 bg-gray-100/30 rounded-xl flex items-center gap-4">
                  <div className="w-10 h-10 rounded-full bg-gradient-to-br from-violet-500 to-cyan-500 flex items-center justify-center flex-shrink-0">
                    <span className="text-gray-900 font-semibold text-sm">{(contact.name || contact.mobile)?.[0]?.toUpperCase() || '?'}</span>
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="font-medium text-gray-900 truncate">{contact.name || 'Sem nome'}</p>
        ...
        ) : <div className="flex-1 flex items-center justify-center text-gray-400"><div className="text-center"><Users className="w-16 h-16 mx-auto mb-4 opacity-50" /><p>Selecione uma lista</p></div></div>}
      </div>
        ...
        {showCreateModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setShowCreateModal(false)}>
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="w-full max-w-md bg-white rounded-2xl border border-slate-800/50" onClick={e => e.stopPropagation()}>
              <div className="p-6 border-b border-slate-800/50"><h2 className="text-xl font-semibold text-gray-900">Nova Lista</h2></div>
              <div className="p-6 space-y-4">
                <div><label className="block text-sm font-medium text-gray-600 mb-2">Nome</label><input type="text" value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} placeholder="Ex: Clientes VIP" className="w-full px-4 py-2.5 bg-gray-50 border border-slate-700/50 rounded-xl text-gray-900 placeholder-slate-400 focus:outline-none focus:border-violet-500/50" /></div>
                <div><label className="block text-sm font-medium text-gray-600 mb-2">Descrição</label><textarea value={formData.description} onChange={e => setFormData({ ...formData, description: e.target.value })} placeholder="Descrição" rows={3} className="w-full px-4 py-2.5 bg-gray-50 border border-slate-700/50 rounded-xl text-gray-900 placeholder-slate-400 focus:outline-none focus:border-violet-500/50 resize-none" /></div>
              </div>
      ...
      <AnimatePresence>
        {showAddContactModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setShowAddContactModal(false)}>
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="w-full max-w-md bg-white rounded-2xl border border-slate-800/50" onClick={e => e.stopPropagation()}>
              <div className="p-6 border-b border-slate-800/50"><h2 className="text-xl font-semibold text-gray-900">Adicionar Contato</h2></div>
              <div className="p-6 space-y-4">
                <div><label className="block text-sm font-medium text-gray-600 mb-2">Nome</label><input type="text" value={contactForm.name} onChange={e => setContactForm({ ...contactForm, name: e.target.value })} placeholder="Nome" className="w-full px-4 py-2.5 bg-gray-50 border border-slate-700/50 rounded-xl text-gray-900 placeholder-slate-400 focus:outline-none focus:border-violet-500/50" /></div>
                ...
                <div><label className="block text-sm font-medium text-gray-600 mb-2">Email</label><input type="email" value={contactForm.email} onChange={e => setContactForm({ ...contactForm, email: e.target.value })} placeholder="email@exemplo.com" className="w-full px-4 py-2.5 bg-gray-50 border border-slate-700/50 rounded-xl text-gray-900 placeholder-slate-400 focus:outline-none focus:border-violet-500/50" /></div>
              </div>
              <div className="p-6 border-t border-slate-800/50 flex gap-3">
      ...
      <AnimatePresence>
        {showImportModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setShowImportModal(false)}>
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="w-full max-w-lg bg-white rounded-2xl border border-slate-800/50" onClick={e => e.stopPropagation()}>
              <div className="p-6 border-b border-slate-800/50"><h2 className="text-xl font-semibold text-gray-900">Importar CSV</h2></div>
              <div className="p-6 space-y-4">
                <div className="p-4 bg-gray-100/30 rounded-xl border border-dashed border-slate-700/50 text-center">
                  <FileSpreadsheet className="w-10 h-10 mx-auto text-gray-500 mb-3" />
                  <p className="text-sm text-gray-500 mb-3">Selecione um arquivo CSV</p>
                ...
                <div className="text-xs text-gray-400"><p className="font-medium mb-1">Formato:</p><p>nome,telefone,email</p></div>
              ...
              <div className="p-6 border-t border-slate-800/50 flex gap-3">
                <button onClick={() => { setShowImportModal(false); setCsvData('') }} className="flex-1 px-4 py-2.5 bg-gray-100 text-gray-600 rounded-xl hover:bg-gray-200">Cancelar</button>
