...
const statusConfig: Record<string, { label: string; color: string; icon: any }> = {
  draft: { label: 'Rascunho', color: 'bg-gray-200 text-gray-600', icon: Clock },
  scheduled: { label: 'Agendada', color: 'bg-blue-500/10 text-blue-400', icon: Calendar },
  running: { label: 'Enviando', color: 'bg-brand-50 text-brand-600', icon: Play },
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="p-4 bg-gray-50 border border-gray-200 rounded-xl">
            <div className="flex items-center gap-3">
              <div className="p-2 bg-brand-50 rounded-lg"><BarChart3 className="w-5 h-5 text-brand-600" /></div>
              <div><p className="text-2xl font-bold text-gray-900">{metrics.totalCampaigns}</p><p className="text-xs text-gray-500">Campanhas</p></div>
            </div>
          </div>
          <div className="p-4 bg-gray-50 border border-gray-200 rounded-xl">
            <div className="flex items-center gap-3">
              <div className="p-2 bg-blue-500/10 rounded-lg"><Send className="w-5 h-5 text-blue-400" /></div>
              <div><p className="text-2xl font-bold text-gray-900">{formatNumber(metrics.totalSent)}</p><p className="text-xs text-gray-500">Enviadas</p></div>
            </div>
          </div>
          <div className="p-4 bg-gray-50 border border-gray-200 rounded-xl">
            <div className="flex items-center gap-3">
              <div className="p-2 bg-green-500/10 rounded-lg"><CheckCircle className="w-5 h-5 text-green-400" /></div>
              <div><p className="text-2xl font-bold text-gray-900">{metrics.totalSent > 0 ? ((metrics.totalDelivered / metrics.totalSent) * 100).toFixed(0) : 0}%</p><p className="text-xs text-gray-500">Entregues</p></div>
            </div>
          </div>
          <div className="p-4 bg-gray-50 border border-gray-200 rounded-xl">
            <div className="flex items-center gap-3">
              <div className="p-2 bg-cyan-500/10 rounded-lg"><Eye className="w-5 h-5 text-cyan-400" /></div>
              <div><p className="text-2xl font-bold text-gray-900">{metrics.totalDelivered > 0 ? ((metrics.totalRead / metrics.totalDelivered) * 100).toFixed(0) : 0}%</p><p className="text-xs text-gray-500">Lidas</p></div>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="flex gap-2">
          {['all', 'draft', 'scheduled', 'running', 'completed'].map((status) => (
            <button key={status} onClick={() => setStatusFilter(status)}
              className={`px-4 py-2 text-sm font-medium rounded-lg transition-all ${statusFilter === status ? 'bg-primary-500 text-white' : 'bg-gray-50 text-gray-500 hover:text-white'}`}>
              {status === 'all' ? 'Todas' : statusConfig[status]?.label || status}
            </button>
          ))}
        </div>
        <button onClick={fetchCampaigns} disabled={isLoading} className="p-2.5 bg-gray-50 border border-gray-200 rounded-xl text-gray-500 hover:text-white transition-colors disabled:opacity-50">
      ...
      <div className="bg-gray-50 border border-gray-200 rounded-2xl overflow-hidden">
        {isLoading && campaigns.length === 0 ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-gray-500">
            ...
            <p className="text-sm text-gray-400 mt-1">Crie sua primeira campanha para começar</p>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-200">
                <th className="text-left py-4 px-6 text-xs font-medium text-gray-500 uppercase">Campanha</th>
                <th className="text-left py-4 px-4 text-xs font-medium text-gray-500 uppercase">Status</th>
                <th className="text-center py-4 px-4 text-xs font-medium text-gray-500 uppercase">Audiência</th>
                <th className="text-center py-4 px-4 text-xs font-medium text-gray-500 uppercase">Enviadas</th>
                <th className="text-center py-4 px-4 text-xs font-medium text-gray-500 uppercase">Entrega</th>
                <th className="text-center py-4 px-4 text-xs font-medium text-gray-500 uppercase">Lidas</th>
                <th className="text-right py-4 px-6 text-xs font-medium text-gray-500 uppercase">Ações</th>
              </tr>
            </thead>
            <tbody>
                  ...
                  <motion.tr key={campaign.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                    ...
                    <td className="py-4 px-6 text-right">
                      <div className="relative">
                        <button onClick={(e) => { e.stopPropagation(); setOpenDropdown(openDropdown === campaign.id ? null : campaign.id) }} className="p-2 hover:bg-gray-100 rounded-lg transition-colors">
