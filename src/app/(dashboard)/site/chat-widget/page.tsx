  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        <button onClick={() => router.push('/site')} className="p-2 rounded-xl bg-gray-50 hover:bg-gray-50 text-gray-500 hover:text-white transition-colors">
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${
                    isActive ? 'bg-gray-50 text-white' : 'text-gray-500 hover:text-gray-700'
                  }`}
                >
            ...
            <div className="bg-white rounded-2xl border border-gray-200 p-6 space-y-5">
              <div>
                ...
                <div className="flex items-center gap-3">
                  <input type="color" defaultValue="#F26B2A" className="w-10 h-10 rounded-lg border border-gray-200 cursor-pointer" />
                  <input type="text" defaultValue="#F26B2A" className="bg-gray-50 border border-gray-200 rounded-xl px-4 py-2.5 text-sm text-gray-700 font-mono w-32 focus:outline-none focus:border-brand-500 transition-colors" />
                </div>
              </div>
              <div>
                <label className="block text-xs text-gray-500 mb-1.5 font-medium">Mensagem de Boas-vindas</label>
                <textarea rows={3} defaultValue="Olá! Como posso ajudar você hoje?" className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm text-gray-900 placeholder:text-gray-500 focus:outline-none focus:border-brand-500 transition-colors resize-none" />
              </div>
              <div>
                <label className="block text-xs text-gray-500 mb-1.5 font-medium">Posição do Widget</label>
                <div className="flex items-center gap-3">
                  <button className="px-4 py-2.5 bg-brand-500/10 border-2 border-[#F26B2A] rounded-xl text-sm font-medium text-[#F26B2A]">Direita</button>
                  <button className="px-4 py-2.5 bg-gray-50 border-2 border-gray-200 rounded-xl text-sm font-medium text-gray-500 hover:border-gray-200 transition-colors">Esquerda</button>
                </div>
              </div>
              <button className="px-4 py-2.5 bg-brand-500 hover:bg-brand-600 text-white text-sm font-medium rounded-xl hover:opacity-90 transition-opacity">Salvar</button>
            </div>
          )}
            ...
            <div className="bg-white rounded-2xl border border-gray-200 p-6 space-y-4">
              {[
                ...
                <div key={item.label} className="flex items-center justify-between p-4 bg-gray-50 rounded-xl border border-gray-200">
                  ...
                  <button className={`w-11 h-6 rounded-full transition-colors ${item.enabled ? 'bg-brand-500' : 'bg-gray-200'}`}>
              ...
              {['Segunda a Sexta', 'Sábado', 'Domingo'].map((day) => (
                <div key={day} className="flex items-center justify-between p-4 bg-gray-50 rounded-xl border border-gray-200">
                  ...
                  <div className="flex items-center gap-2">
                    <input type="time" defaultValue="09:00" className="bg-white border border-gray-200 rounded-lg px-3 py-1.5 text-xs text-gray-600" />
                    <span className="text-gray-500">até</span>
                    <input type="time" defaultValue="18:00" className="bg-white border border-gray-200 rounded-lg px-3 py-1.5 text-xs text-gray-600" />
                  </div>
                </div>
              ))}
              <button className="px-4 py-2.5 bg-brand-500 hover:bg-brand-600 text-white text-sm font-medium rounded-xl hover:opacity-90 transition-opacity">Salvar Horários</button>
            </div>
          )}
          ...
          {activeTab === 'install' && (
            <div className="bg-white rounded-2xl border border-gray-200 p-6 space-y-4">
              ...
              <div className="relative">
                ...
                <button
                  onClick={handleCopy}
                  className="absolute top-3 right-3 p-2 rounded-lg bg-gray-50 hover:bg-gray-50 text-gray-500 hover:text-white transition-colors"
                >
