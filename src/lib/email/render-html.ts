...
function renderBlock(block: EmailBlock, font: string, settings?: EmailDocument['settings']): string {
  ...
  switch (block.type) {
    ...
    case 'coupon':
      return `<tr><td style="padding:${blockPad};background-color:${p.backgroundColor || '#FFF7ED'};text-align:center;font-family:${font};">${p.headerText ? `<p style="margin:0 0 10px;font-size:${p.headerFontSize || 14}px;color:${p.headerColor || '#9A3412'};font-weight:500;">${p.headerText}</p>` : ''}<p style="margin:0;font-size:${p.codeFontSize || 32}px;font-weight:${p.codeFontWeight || 'bold'};color:${p.codeColor || '#EA580C'};letter-spacing:${p.codeLetterSpacing ?? 4}px;${p.borderStyle !== 'none' ? `border:${p.borderWidth ?? 2}px ${p.borderStyle || 'dashed'} ${p.borderColor || '#EA580C'};` : ''}border-radius:${p.borderRadius || 12}px;display:inline-block;padding:10px 28px;${p.codeBgColor ? `background-color:${p.codeBgColor};` : ''}">${p.code || ''}</p>${p.footerText ? `<p style="margin:10px 0 0;font-size:${p.footerFontSize || 12}px;color:${p.footerColor || '#9CA3AF'};">${p.footerText}</p>` : ''}</td></tr>`
