    ...
    def test_updating_a_tenant_touches_updated_at(self, admin: psycopg.Connection) -> None:
        ...
        try:
            before = admin.execute(
                "select updated_at from public.tenants where id = %s", (organization_id,)
            ).fetchone()[0]

            admin.execute(
                "update public.tenants set name = 'touched' where id = %s", (organization_id,)
            )
            after = admin.execute(
                "select updated_at from public.tenants where id = %s", (organization_id,)
            ).fetchone()[0]
        finally:
            admin.execute("delete from public.tenants where id = %s", (organization_id,))
